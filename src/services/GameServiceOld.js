...
module.exports = class GameService {
    ...
    try {
      ...
      for (var i = 0; i < fakePlayers.length; i++) {
        ...
        let tempPlayer = {
          ...
          targetXP:(level<12?Facker.targetXp[level]:(currentLevelXP>=600?600:currentLevelXP+50)),
      ...
      let getXPInfo = collectXP(match, user, false);
      // let game = await Game.findOne();
      ...
      user.level = getXPInfo.currentLevel;
      user.currentLevelXP = getXPInfo.currentLevelXP;
      user.currentXP = getXPInfo.currentXP;
      user.totalPlayedMatch = parseInt(user.totalPlayedMatch) + 1;
      let playHistory = [...user.playHistory, gamePlayInfo.playHistory];
  ...
  async gameOver(body, user) {
    ...
    let updatedPlayHistory=[...player.playHistory];
    let totalPlayHistoryLength=player.playHistory.length-1;

    updatedPlayHistory[totalPlayHistoryLength]=gamePlayInfo.playHistory;

    player.playHistory = updatedPlayHistory;

    let result = await player.save();
    
    let totalPlayedMatch = player.totalPlayedMatch;
    ...
    let data = {
      player: {
        ...
        targetXP:(result.level<12?Facker.targetXp[result.level]:(result.currentLevelXP>=600?600:result.currentLevelXP+50)),
        level: result.level,
